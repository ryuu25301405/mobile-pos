...
export default function DailySalesReportPage() {
  ...
  // Store Performance Breakdown (When "ALL" stores selected)
  ...
  // Hourly Breakdown + Peak Hour Badge Calculation
  const hourlyData = useMemo(() => {
    ...
    const hourlySkus: Record<number, Record<string, number>> = {};
    ...
    filteredSalesData.forEach((item) => {
      ...
      hours[h].revenue += price;
      hours[h].count += qty;

      const sku = item.sku || item.style_code || "N/A";
      if (!hourlySkus[h]) hourlySkus[h] = {};
      hourlySkus[h][sku] = (hourlySkus[h][sku] || 0) + qty;
    });

    // Find top SKU per hour
    hours.forEach((h) => {
      const skus = hourlySkus[h.hourNumber];
      if (skus) {
        const sorted = Object.entries(skus).sort((a, b) => b[1] - a[1]);
        if (sorted.length > 0) h.topSKU = sorted[0][0];
      }
    });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      {/* Store Breakdown Cards (When ALL stores selected) */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Hourly Chart with Popup Tooltips */}
        <div className="lg:col-span-2 bg-slate-900/60 border border-slate-800 rounded-xl p-5 space-y-4">
          ...
          <div className="h-52 flex items-end gap-1.5 pt-10 pb-2 px-2 overflow-x-auto relative">
            {hourlyData.hours.map((item, idx) => (
              ...
              >
                <div className="relative w-full flex justify-center">
                  ...
                  {/* Hover Popup Card */}
                  {activeHourIndex === idx && (
                    <div className="absolute -top-20 bg-slate-900 border border-indigo-500/40 text-slate-100 text-xs p-2.5 rounded-lg shadow-2xl z-30 min-w-[140px] pointer-events-none animate-in fade-in zoom-in-95 duration-150">
                      <p className="font-semibold text-indigo-300 border-b border-slate-800 pb-1 mb-1">
                        Time: {item.hourLabel}
                      </p>
                      <p className="text-emerald-400 font-bold">
                        ₱{item.revenue.toLocaleString("en-PH", { minimumFractionDigits: 2 })}
                      </p>
                      <p className="text-[11px] text-slate-400">Items Sold: {item.count}</p>
                      {item.topSKU && (
                        <p className="text-[10px] text-slate-500 truncate mt-0.5">Top: {item.topSKU}</p>
                      )}
                    </div>
                  )}
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-3">
            {topProducts.length === 0 ? (
              <p className="text-sm text-slate-500 py-4 text-center">No sales recorded for this date.</p>
            ) : (
              topProducts.map((prod, idx) => (
                <div
                  key={prod.sku}
                  className="flex items-center justify-between p-2.5 rounded-lg bg-slate-800/40 border border-slate-800 hover:border-slate-700 transition-colors"
                >
