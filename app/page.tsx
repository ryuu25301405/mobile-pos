...
export default function Home() {
  ...
  // Click handler to open scanner camera
  ...
  // Handle barcode/QR code processing
  const handleScan = async (scannedBarcode: string) => {
    ...
    if (error || !data) {
      ...
      // Save scan to database including store location
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-center items-center p-4 sm:p-6 antialiased">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6 text-center">
        {/* Title */}
        <div className="space-y-1">
          <h1 className="text-2xl font-black text-white tracking-tight">Barcode & QR Scanner</h1>
          <p className="text-xs text-slate-400">Select a store location and start scanning</p>
        </div>
        ...
        <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 text-left space-y-2">
          <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">
            Select Store Location <span className="text-red-400">*</span>
          </label>
          <select
            value={selectedStore}
            onChange={(e) => setSelectedStore(e.target.value)}
            className="w-full bg-slate-900 border border-slate-700/80 text-white rounded-xl p-3 text-sm focus:outline-none focus:border-emerald-500 font-semibold cursor-pointer"
          >
        ...
        <button
          ...
          className="w-full py-4 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-extrabold rounded-2xl text-sm flex items-center justify-center space-x-2 transition active:scale-95 shadow-lg shadow-emerald-500/20"
        >
      ...
      {/* Store Prompt Modal (Triggers if Scan button is tapped before picking store) */}
      {isStoreModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-sm shadow-2xl space-y-5 text-left">
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-400">Please pick your store location before opening camera.</p>
            </div>
      ...
      {scanning && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex flex-col items-center justify-center p-4">
          <div className="relative w-full max-w-md bg-slate-900 rounded-3xl overflow-hidden border border-slate-800 shadow-2xl">
            {/* Header info bar displaying active store location */}
                  ...
                  {errorMessage ? (
                    ...
                    scannedProduct && (
                      <div className="space-y-2 w-full max-w-xs">
                        ...
                        <h3 className="text-lg font-bold text-white leading-tight">{scannedProduct.styleName}</h3>
                        <p className="text-xs font-mono text-emerald-400 font-bold">{scannedProduct.styleCode}</p>
                      </div>
                    )
                  )}
