...
export default function MobileScanPage() {
  ...
  // Handle parsing / searching scanned code and direct auto-saving
  const handleScanSubmit = async (e: React.FormEvent) => {
    ...
    // Auto-save entry directly to scanned_logs
    const logPayload = {
      store: selectedStore,
      style_code: itemData.styleCode,
      sku: itemData.sku !== "-" ? itemData.sku : null,
      style_name: itemData.styleName,
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 p-4 max-w-md mx-auto antialiased space-y-4 pb-12">
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-2xl p-3.5 space-y-2">
        ...
        <form onSubmit={handleScanSubmit} className="flex space-x-2">
          ...
          <button
            type="submit"
            disabled={!selectedStore || !scannedInput.trim() || isSubmitting}
      ...
      {scannedItem && (
        <section className="bg-slate-900 border border-emerald-500/40 rounded-2xl p-4 space-y-4 shadow-xl shadow-emerald-500/5">
          ...
          <div className="space-y-2">
            {/* Style Code & SKU Display Badges */}
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-xs font-mono font-bold bg-slate-950 border border-slate-800 text-emerald-400 px-2.5 py-1 rounded-lg">
