...
export default function ScanViewPage() {
      ...
      .on(
        ...
        (payload) => {
          ...
          const formattedNewItem: ScannedProduct = {
            ...
            store: newItem.store || "Unassigned Store",
  ...
  // Click handler for opening scanner
  ...
  // Confirm store choice from modal
  ...
  const handleScan = async (scannedBarcode: string) => {
    ...
    if (error || !data) {
      ...
      // Save scan record including the selected store
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-white mt-0.5">
                {selectedStore ? (
                  <span className="text-emerald-400">{selectedStore}</span>
                ) : (
                  <span className="text-slate-500 italic">No store selected (Required to scan)</span>
                )}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <div className="col-span-2 sm:col-span-1 bg-slate-900 border border-slate-800 rounded-2xl p-3.5 shadow-md flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-emerald-400 mt-1 font-semibold">Realtime Active</p>
            </div>
            <button
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-5">
            <div className="space-y-1">
              <h2 className="text-lg font-bold text-white">Select Current Store Location</h2>
              <p className="text-xs text-slate-400">
                You must choose a store location before scanning items.
              </p>
            </div>
        ...
        {pageLoading ? (
          ...
          <div className="bg-slate-900/50 border border-slate-800/80 rounded-3xl p-12 text-center my-auto space-y-3">
            ...
            <p className="text-xs text-slate-400 max-w-sm mx-auto">
              Select your store and tap "Scan Barcode / QR" to log items.
            </p>
          </div>
        ) : (
              ...
              {scannedItems.map((item) => (
                ...
                >
                  <div className="sm:col-span-5 space-y-1">
                    <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-md">
                      {item.store}
                    </span>
